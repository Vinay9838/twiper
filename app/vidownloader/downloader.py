...
def run_ffget(ffget_path: Path, url: str, output_path: Path, dry_run: bool) -> int:
	...
	proc = subprocess.run(
		cmd,
		stdin=subprocess.DEVNULL,   # 🔑 prevents FFmpeg interactive mode
		stdout=sys.stdout,
		stderr=sys.stderr,
		check=False,
	)
...
def main() -> int:
	parser = argparse.ArgumentParser(
		description="Read videos.json and download videos via ffget.sh"
	)
	parser.add_argument(
		"--json",
		default=(BASE_DIR / "data/videos.json"),
	...
	parser.add_argument(
		...
		type=int,
		default=None,
		help="Starting number (overrides auto-detect)",
	)
	parser.add_argument(
		"--limit",
		type=int,
	...
	start_num = (
		args.start if args.start and args.start > 0
		else next_start_number(base_dir)
	)
